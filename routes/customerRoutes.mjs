...
router.post('/orders/add', protectCustomer, async (req, res) => {
  try {
    ...
    currentDateTime.setHours(0, 0, 0, 0); // Set the time to midnight for the current date
    
    // Find the reservation where the date matches the year, month, and day of the current date
    const existingReservation = await Reservation.findOne({
      date: {
        $gte: currentDateTime.toISOString(),
        $lt: new Date(currentDateTime.getTime() + 24 * 60 * 60 * 1000).toISOString()
      },
      customerId: userId
    });
    ...
    if (existingReservation) {
      // Find user using userId from users collection
      const user = await User.findOne({ _id: userId });

      if (user) {
        // Add the entire user document in the Order
        const newOrder = await Order.create({ menuId, userId, user, payment });
